*! version 1.7.1  27jan2012
program clogit_buc, eclass byable(onecall) prop(swml or svyb svyj svyr mi fvaddcons)
	if _caller() >= 11 {
...
// Parse. 
	...
	syntax varlist(numeric ts fv) [fw iw pw] [if] [in] [, ///
		...
		noLOg noDISPLAY noHEADer SCore(string) noNEST ///
		VCE(passthru) DOOPT* ]
...
//AB make quiet
	`vv' ///
	qui _rmcoll `xvars' if `touse' `wgt', `coll' `fvexp'
...
// Count clusters once offending observations have been removed.
	...
	if "`robust'" != "" {
//AB modify 1 line below
		local critopt "crittype(log conditional likelihood)"
	}
...
//AB change title
	local title "Fixed-effects ordered logistic regression"
...
// Maximize.
	...
	ml model `mm' clogit_lf (`depname2' `depvar' = `xvars', ///
		...
		`noscore' title("`title'") `critopt' `gropt' `negh'
...
//AB insert 2 line below
	_coef_table_header_buc 
//AB modify header 1 line below
...
//AB insert 1 line below
	eret local depvar $gdepvar
end
