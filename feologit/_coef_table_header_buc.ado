...
program _coef_table_header_buc
	version 9
	if !c(noisily) {
		exit
	}
...
//AB modify label
	if !missing(`e'(N)) {
		.`right'.Arrpush				///
			`C3' "N. of obs. (inc. copies)" `C4' "= "		///
			as res %`c4wfmt'.0f `e'(N)
	}

//AB add clause for genuine observations
		if !missing(`e'(N_true)) {
		.`right'.Arrpush				///
			`C3' "N. of observations" `C4' "= "		///
			as res %`c4wfmt'.0f `e'(N_true)
	}
	
//AB modify if clause below so number of groups used in estimation shown as individuals/units	
	if !`is_svy' & !missing(`e'(N_group)) { 
	 //& "`cluster'``e'(clustvar)'" == "" {
		if "`scheme'" != "svy" {
...
//AB modify Display name 1 line below
	Display_buc `left' `right' `"`title'"' `"`title2'"'
